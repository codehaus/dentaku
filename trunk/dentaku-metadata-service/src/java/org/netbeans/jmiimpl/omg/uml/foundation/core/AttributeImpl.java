...
abstract public class AttributeImpl extends ModelElementImpl implements
		Attribute {
	 ...
	 * @param follow
	 *            <b>true </b> if search should follow inheritance hierarchy
	...
	public String findTagValue(String tagName, boolean follow) {
		...
		for (ClassifierImpl element = (ClassifierImpl) getType(); value == null
				&& element != null; element = (ClassifierImpl) element
				.getJavaGeneralization()) {
	...
	public String getAttributeSQLType() {
		...
		if (null == value) {
			value = ((ModelElementImpl) getType()).getFullyQualifiedName();
			if (typeMappings != null) {
				value = typeMappings.getSQLType(value,
						getAttributeSQLFieldLength());
			}
		}
		return value;
	}
