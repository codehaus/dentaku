...
public class PDependency
    ...
    public static Dependency newInstance(UMLStaticHelper scriptHelper,
                                         Dependency dependency) {
        Class[] interfaces = {
            UMLDependency.class,
            Dependency.class
        };
        return (Dependency) java.lang.reflect.Proxy.newProxyInstance(dependency.getClass().getClassLoader(),
                interfaces,
                new PDependency(dependency, scriptHelper));
    }

    protected PDependency(Dependency dependency,
    ...
    public ModelElement getTargetType() {
        ...
        if (supplier instanceof Attribute) {
            return PAttribute.newInstance(scriptHelper, (Attribute) supplier);
        }
        if (supplier instanceof Operation) {
            return POperation.newInstance(scriptHelper, (Operation) supplier);
        }
        if (supplier instanceof Classifier) {
            return PClassifier.newInstance(scriptHelper, (Classifier) supplier);
        }
        return PModelElement.newInstance(scriptHelper, supplier);
    }
}
