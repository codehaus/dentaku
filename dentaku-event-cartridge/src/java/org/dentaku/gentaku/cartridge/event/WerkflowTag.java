...
public class WerkflowTag extends TagSupport {
    ...
    public void setUmlUtils(UMLUtils umlUtils) {
        this.umlUtils = umlUtils;
    }
    ...
    Document generateDocument(Object modelElement) throws JellyTagException {
        ...
        try {
            ag = (ActivityGraph)umlUtils.findOneItem(((Namespace)modelElement), ActivityGraph.class);
        } catch (GenerationException e) {
            throw new JellyTagException(e);
        }
        Element seq = process.addElement("sequence");
        Pseudostate startState = umlUtils.findInitialState(((CompositeState) ag.getTop()).getSubvertex());
        if (startState != null) {
            ...
            for (Iterator it = topological.iterator(); it.hasNext();) {
                ...
                if (vertex instanceof SimpleState) {
                    String vertexClass = umlUtils.getDestinationFullyQualifiedClassName(modelElement);
